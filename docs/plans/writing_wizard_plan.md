...
# 本地部署
git clone -b feature/rich-text-editor https://github.com/greatfeel/deer-flow
项目需求参考writing_wizard_plan.md文件，执行下面的命令

## 安装
make check
make config
make install
把服务器上面的 config.yaml 和 .env 拉下来使用
...
# 服务器部署
- 安装是一样的，已经放在 /data/projects/solution_agent_server
- git pull 更新，然后直接运行 make start 前台运行，千问不要 make dev
Access: http://ido.modelturbo.com:2026/

- 如果是后台启动，运行 ./scripts/start-daemon.sh 