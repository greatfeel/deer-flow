...
# 2026-03-31-02
治疗方案不需要是一个完整的方案，应该聚焦在armGroups和interventions模块，参考 /doc/plans/writing_wizard_plan.md
中治疗方案部分的两个例子

# 2026-03-31-01
## 目前前端设计已经基本满意，下面我们实现后端逻辑，为了减少不确定性，我们先实现一部分功能
- 实现从 1）研究题目 到 10）治疗方案的后台逻辑
- 其中关键词的后台逻辑暂不实现
- 暂不需要后端持久化，在 session 中存储中间结果即可
